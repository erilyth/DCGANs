...
"""
# Algorithm Specifics:
1) Uses the Olivetti faces dataset containing 400 images of 40 different people
...
discriminator.add(Convolution2D(32, 5, 5, border_mode='same', input_shape=(1,64,64)))
discriminator.add(AveragePooling2D(pool_size=(2, 2), padding='valid'))
...
# Apply log loss at the output of shape (_, 1, 64, 64)
...
def train_gan():
    ...
    for time_step in tqdm(range(100000)):
        ...
        random_noise2 = np.random.normal(loc=0.0, scale=1.0, size=[batch_size*2, 100])
        gen_current_train = random_noise2.copy()
